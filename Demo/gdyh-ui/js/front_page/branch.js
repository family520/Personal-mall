$(function() {
			...
			success: function(res) {
				if(res.code == 0) {
					...
					if(resData == null || resData.list.length == 0) {
						...
						document.getElementById("pmOnePointZero").innerHTML = "-" + " " + '<span class="unit"></span>';
					} else {
						$('.item>span').removeClass('bac-red').removeClass('bac-red-linear');
						...
						if(isLora == 1) {
							...
							let pmOneZero = resData.list[0].pmOnePointZero;
							document.getElementById("co2").innerHTML = resData.list[0].coTwo + " " + '<span class="unit">ppm</span>';
							document.getElementById("formaldehyde").innerHTML = resData.list[0].hcho + " " + '<span class="unit">ppb</span>';
							document.getElementById("voc").innerHTML = resData.list[0].voc + " " + '<span class="unit">ppb</span>';
							document.getElementById("temperature").innerHTML = temperatureNum + " " + '<span class="unit">℃</span>';
							document.getElementById("humidity").innerHTML = humidityNum + " " + '<span class="unit">%RH</span>';
							...
							document.getElementById("pmOnePointZero").innerHTML = pmOneZero + " " + '<span class="unit">μg/m³</span>';
							document.getElementById("wendu").innerHTML = temperatureNum + "℃";
							document.getElementById("shidu").innerHTML = humidityNum + "%";
							initguageCharts('echarts3', temperatureNum, -50, 50);
							initguageCharts('echarts4', humidityNum, 0, 100);
		...
		$.get(deviceServiceUrl + "/device/getConsumeTotal?token=" + $.getToken() + "&consumeTime=" + ymd + "&organizationId=" + organizationId, function(totalRes) {
			if(totalRes.code == 0) {
				if(totalRes.data == null || totalRes.data.length == 0) {
					...
					vm.consumeTotal = 0;
					//vm.consumeByDevice = ["0%", "0%", "0%", "0%", "0%", "0%", "0%"];
					return;
				}
				let dataXList = [];
				let echartDataList = [];
				...
				//let consumeByDevice = [0, 0, 0, 0, 0,0,0]; //"空调","照明","应急","风机","灯箱","UPS电源","其它"
				for(let i = 0; i < totalRes.data.length; i++) {
					...
					} else if(totalRes.data[i].devicePositionName == "应急") {
						consumeByDevice[2] += consumeFloat;
					} else if(totalRes.data[i].devicePositionName == "风机") {
						consumeByDevice[3] += consumeFloat;
					} else if(totalRes.data[i].devicePositionName == "灯箱") {
						consumeByDevice[4] += consumeFloat;
					} else if(totalRes.data[i].devicePositionName == "UPS电源") {
						consumeByDevice[5] += consumeFloat;
					} else if(totalRes.data[i].devicePositionName == "其它") {
						consumeByDevice[6] += consumeFloat;
					}*/
				}
				//获取各类设备耗能百分比
				/*if(consumeTotal == 0) {
					consumeByDevice = ["0%", "0%", "0%", "0%", "0%", "0%", "0%"];
				} else {
	...
	function getCameraPic() {
		...
		$.get(deviceServiceUrl + "/camera/getCameraPic?token=" + $.getToken() + "&organizationId=" + organizationId, function(res) {
	...
	$(function() {
		...
		$("#echarts2").contents().css("width", "4.2rem");
		$("#echarts2").contents().css("height", "2rem");
		$("#echarts2").contents().contents().css("width", "4.2rem");
		$("#echarts2").contents().contents().css("height", "2rem");
